...
export default function GlobalWalletBar({ inline = false }: { inline?: boolean }) {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!offer && pendingOfferId) {
          ...
          offer = (typeof data === 'string' && data.startsWith('offer')) ? data : (data.offer?.offer || data.offer || data?.data?.offer)
        }
  ...
  const boxStyle: React.CSSProperties = inline ? {
    position: 'static', display: 'inline-flex', alignItems: 'center', gap: isMobile ? 6 : 8, padding: 0, background: 'transparent'
  } : {
    position: 'fixed', top: topOffset, right: rightOffset, zIndex: layer,
  ...
  const panelStyle: React.CSSProperties = {
    ...
    boxShadow: '0 12px 26px rgba(0,0,0,0.18)', padding: isMobile ? 10 : 14,
    minWidth: isMobile ? 220 : 300, maxWidth: isMobile ? 300 : 380
  }
