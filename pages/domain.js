...
import { getCachedUserOffers } from '../lib/offers-cache'
...
export async function getServerSideProps(ctx) {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      // Fetch offers for this user (cached, best-effort; swallow errors so profile still renders)
      try {
        userOffers = await getCachedUserOffers(username)
      } catch (offersErr) {
        console.warn('Offers fetch failed (non-fatal)', offersErr)
        userOffers = []
      }
    }
  } catch (e) {
  ...
  return { props: { user, ownedPfps, otherOwners, ownedHasMore, othersHasMore, userOffers, pageSize: PAGE_SIZE, rootHostForLinks, ownedCount, othersCount, initialQuery: searchQ } }
}
...
export default function DomainPage({ user, ownedPfps = [], otherOwners = [], ownedHasMore = false, othersHasMore = false, userOffers = [], pageSize = 60, rootHostForLinks, ownedCount = 0, othersCount = 0, initialQuery = '' }) {
  ...
  // Calculate offers count from actual offers array to ensure consistency
  const actualOffersCount = useMemo(() => {
    return Array.isArray(userOffers) ? userOffers.length : 0
  }, [userOffers])
  ...
  const [collectionTab, setCollectionTab] = useState('my') // 'my' | 'others' | 'offers'
...
Claim on <span aria-hidden="true" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 20, fontSize: 18, fontWeight: 800}}>𝕏</span>
  ...
  <div style={{ marginTop: 30, width: '100%', maxWidth: 1100, marginLeft: 'auto', marginRight: 'auto', alignSelf: 'stretch' }}>
          ...
          {collectionTab !== 'offers' && (collectionTab === 'my' ? ownedMore : othersMore) && !intersectionSupported && (
